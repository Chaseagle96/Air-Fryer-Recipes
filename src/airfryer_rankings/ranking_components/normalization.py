...
def category_baselines(current: list[dict], global_prior: float, params: ModelParameters) -> dict[str, dict]:
    ...
    for category, items in grouped.items():
        ...
        raw_mean = sum(rating * weight for rating, weight in zip(ratings, weights, strict=True)) / max(1e-9, sum(weights))
...
def source_adjustments(
    ...
    for source, pairs in grouped.items():
        ...
        raw_residual = sum(value * weight for value, weight in zip(residuals, weights, strict=True)) / max(1e-9, sum(weights))
