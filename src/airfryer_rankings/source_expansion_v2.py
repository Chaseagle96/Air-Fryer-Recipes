...
# Preserve the v1 implementations so the compatibility adapter can enrich their
# stable discovery/qualification machinery without recursively calling itself.
...
def qualification_metrics(
    ...
    *,
    ...
    """Gate-v2 source metrics with rating availability separated from extraction.

    Recipe Intelligence's production ranking extractor intentionally emits a
    ranking row only when usable rating/review evidence exists. Gate v1 divided
    extracted ranking rows by every recognized recipe page, which made a
    rating-free editorial recipe look like an extractor failure. V2 instead
    measures extractor reliability *conditional on ranking evidence being
    present*, while separately reporting how much of the source can currently
    contribute rating-backed rows to Bayesian ranking.
    """
    ...
    # has_rating captures validated Recipe aggregateRating evidence; a successful
    # production extraction also proves ranking evidence existed via either schema
    # or the supported visible-evidence fallback.
...
def score_source_quality(
    ...
    """Score a publisher without making public star ratings an admission requirement."""
    ...
    if conditional_extraction is None:
        # No rating/review evidence means technical extraction reliability is not
        # observable. Redistribute that weight across recipe-structure signals so
        # rating-free editorial/test-kitchen publishers are neither rewarded nor
        # punished for lacking public star ratings.
...
def _load_contexts(config: dict[str, Any]) -> list[_legacy.VerticalContext]:
    ...
    for context in contexts:
        # This is the current decision model version. Historical audit events keep
        # their original source_gate_version values, so no prior admission reason
        # is rewritten when the model evolves.
...
# Public compatibility aliases. Discovery, networking, lifecycle, persistence,
# auditability, budgets, and cross-vertical behavior remain in the shared engine.
